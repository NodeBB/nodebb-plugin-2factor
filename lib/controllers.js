...
Controllers.renderLogin = async (req, res, next) => {
	...
	const publicKey = await parent.hasU2f(req.uid) && await parent.getU2fKey(req.uid);
	let challenge;
	if (publicKey) {
		const keyHandle = await parent.getU2fKeyHandle(publicKey);
		challenge = u2f.request(nconf.get('url'), keyHandle);
		req.session.authRequest = challenge;
	}
